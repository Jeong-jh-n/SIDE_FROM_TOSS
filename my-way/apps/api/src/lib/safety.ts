 ...
 * 우리 대상은 **만 19세 이상**(앱인토스 미니앱 기준)이고, 진로·취업 스트레스
 * 상황에 있는 사람이 많아요. 미성년자는 아니지만 위험 신호는 그대로 나옵니다.
 * (CLAUDE.md §8)
 *
 * ## 두 가지를 합니다
 *
 ...
 *
 * > **주의**: 같은 파일이 `apps/web/src/lib/safety.ts` 에도 있습니다.
 * > 클라이언트가 걸렀더라도 **서버는 믿지 않고 한 번 더 검사해요.**
 * > 프론트를 우회한 직접 호출이 가능하기 때문입니다. **고칠 때 함께 고치세요.**
 */
