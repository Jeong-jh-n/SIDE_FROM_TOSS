 ...
 * 공개 배포에 필요한 최소 방어.
 *
 * 이 서버는 **커리어넷 인증키를 들고 있습니다.** 배포되면 주소를 아는 누구나
 * 호출할 수 있어요. 매뉴얼에 "이용량에 따라 사용이 제한될 수 있습니다"라고 적혀 있고
 * 키 유효기간은 2년입니다. 남이 우리 키로 긁어가면 서비스가 멈춥니다.
 *
 * 화이트리스트(`ALLOWED_TESTS`)는 **어떤 검사를** 부를 수 있는지를 막고,
 * 여기서는 **얼마나 자주** 부를 수 있는지를 막아요.
 */
